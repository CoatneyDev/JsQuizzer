...
class GameClock {
    constructor(clock, callback, controller) {
    ...
    startTimer() {
        ...
        function updateTimer() {
            ...
            if (counter === TENMINUTES) {
                game.stopTimer();
                game.callback.call(this.controller);
            } else {
...
class QuizController {
    ...
    async init() {
        ...
        this.gameClock = new GameClock(this.quizView.getClock(), this.timeout, this);
    ...
    timeout() {
        quizView.showModal("Time's Up!", "You didn't complete the quiz in time.", "", true);
        ...
        if (!this.playing) { // If we are starting the quiz...
            ...
            try { // Add current choice to user answer set
                ...
                if (typeof selection === "object") {
                    ...
                    // reset selection Closure
                    this.userSelection(-1);

                    this.displayNextQuestion();
                } else {
                    console.log("The button was pressed but the user didn't answer a question. Nothing was recorded.");
                    return;
                }
            } catch (err) {
                console.log("The button was pressed but the user didn't answer a question. Nothing was recorded.");
